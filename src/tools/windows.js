...
async function mutatingCall(action, args) {
  ...
  const current = await callUia({ action: "get_value", ref: args.ref });
  assertProcessAllowed(current.processName);
  return callUia({ action, ...args });
}
...
export function registerWindowsTools(server) {
  ...
  server.registerTool(
    ...
    async ({ ref, maxDepth, maxNodes }) => text(await callUia({ action: "tree", ref, maxDepth, maxNodes }))
  );
  ...
  server.registerTool(
    "windows_find",
    {
      ...
      description: "Searches descendants of the given ref by automationId/name/controlType (any combination), capped at limit.",
      inputSchema: {
        ref: z.string(),
    ...
    async (args) => text(await callUia({ action: "find", ...args }))
  );
  ...
  server.registerTool(
    "windows_wait_for",
    {
      ...
      description: "Polls under the given window ref until an element matching automationId/name/controlType appears, or timeoutMs elapses (returns {found:false}, not an error, on timeout).",
      inputSchema: {
        ref: z.string(),
        automationId: z.string().optional(),
        name: z.string().optional(),
    ...
    async (args) => text(await callUia({ action: "wait_for", ...args }))
  );
  ...
  server.registerTool(
    ...
    async ({ ref }) => text(await callUia({ action: "get_value", ref }))
  );
